...
-(void) __playFrame:(int)_frame forAnimation:(NSString *)_anim {
    ...
    for (unsigned int t=0; t < transfList.count; t++) {
        ...
        if ([_attr isEqualToString:@"r"])
            [self applyRotAtAxis:_axis value:tr.value];
        if ([_attr isEqualToString:@"l"])
            [self applyLocAtAxis:_axis value:tr.value];
        if ([_attr isEqualToString:@"s"])
            [self applyLocAtAxis:_axis value:tr.value];
    }
}
...
-(void) __processFrames {
    ...
    for (NSString *animationName in self.animationTable) {
        ...
        for (unsigned int t=0; t < transformations.count; t++) {
            TDAnimTransformation *tempTransform = [transformations objectAtIndex:t];
            NSString *transType = tempTransform.type;
